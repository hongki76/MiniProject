...
(function (global, $) {
  ...
  (function initSearchInputUX() {
    ...
    function applyPlaceholderAndValidation() {
      ...
      else if (v === "1") { $kw.attr("placeholder", "가격(숫자)"); $kw.attr("num","n"); if ($reg.val()) $kw.val(""); }
  ...
  (function autoCompleteModule() {
    ...
    function ensureMenu() {
      ...
      else { var $p=$kw.parent(); if ($p.css("position")==="static") $p.css("position","relative"); $p.append(menu); }
      return menu;
    }
    ...
    function renderMenu(list){
      ...
      items = list.slice(0); activeIndex=-1;
    ...
    function moveActive(d){ if(!items.length) return; activeIndex=(activeIndex+d+items.length)%items.length; menu.children().removeClass('active').eq(activeIndex).addClass('active').css('background','#f0f6ff'); }
    function applyChoice(i){ if(i<0||i>=items.length) return; $kw.val(items[i]).trigger('input'); hideMenu(); }
    function debounce(fn, wait){ return function(){ clearTimeout(timer); var ctx=this,args=arguments; timer=setTimeout(function(){ fn.apply(ctx,args); }, wait); }; }
    var requestSuggestDebounced = debounce(function(){
      var cond = String($cond.val()||"0");
      var kw = String($kw.val()||"").trim();
      if (cond === "2"){ if(kw.replace(/\D/g,"").length < 1){ hideMenu(); return; } }
      else { if(kw.length < 1){ hideMenu(); return; } }
      var reqId = ++lastReq;
      $.ajax({
        ...
        contentType: "application/json; charset=UTF-8",
        dataType: "json",
        data: JSON.stringify({ searchCondition: cond, keyword: cond === "2" ? kw.replace(/\D/g,"") : kw, limit: 10 })
      }).done(function(list){ if(reqId!==lastReq) return; renderMenu(Array.isArray(list)?list:[]); }).fail(function(){ hideMenu(); });
    }, DEBOUNCE_MS);
    ...
    $kw.on("keydown", function (e) {
      ...
      else if (e.key === "Enter"){ if (activeIndex >= 0){ e.preventDefault(); applyChoice(activeIndex); } }
  ...
  (function hoverPreview(){
    ...
    function formatYMD(v){ if(v==null) return ''; var s=String(v).trim(); if(/^\d{13}$/.test(s)){var d=new Date(Number(s)); if(!isNaN(d)) return [d.getFullYear(),String(d.getMonth()+1).padStart(2,'0'),String(d.getDate()).padStart(2,'0')].join('-');} if(/^\d{10}$/.test(s)){var d2=new Date(Number(s)*1000); if(!isNaN(d2)) return [d2.getFullYear(),String(d2.getMonth()+1).padStart(2,'0'),String(d2.getDate()).padStart(2,'0')].join('-');} if(/^\d{8}$/.test(s)) return s.slice(0,4)+'-'+s.slice(4,6)+'-'+s.slice(6,8); return s; }
    function fmtPriceLocal(v){ var n=Number(v||0); return isNaN(n)?'':n.toLocaleString(); }
    function buildLayerHtml(p){
      var html=''; html+='<div class="ttl">'+escapeHtml(p.prodName||'')+'</div>';
      ...
      html+='<div class="act"><form action="'+(cPath||"")+'/product/getProduct" method="post" style="display:inline;"><input type="hidden" name="prodNo" value="'+p.prodNo+'"/><button type="submit" class="btn-like">상세보기</button></form></div>'; return html;
    }
    function placeLayerNear($a){ var r=$a[0].getBoundingClientRect(); var sx=window.pageXOffset||document.documentElement.scrollLeft; var sy=window.pageYOffset||document.documentElement.scrollTop; $layer.css({top:(r.bottom+sy+6)+'px', left:(r.left+sx)+'px'}); }
    function doShow($a, prodNo){
      ...
      $.getJSON((cPath||'')+'/product/json/getProduct/'+encodeURIComponent(prodNo), function(p){ if(p&&typeof p==='object'){ cache[prodNo]=p; onData(p); } });
    }
    function scheduleShow($a){ clearTimeout(showTimer); clearTimeout(hideTimer); var prodNo=$a.data('prodno'); if(!prodNo) return; showTimer=setTimeout(function(){ doShow($a, prodNo); }, DELAY_SHOW); }
